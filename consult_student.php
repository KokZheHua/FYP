<?php
    ...
    $query_getClassroomID = "SELECT student_classroom_id FROM student_classroom WHERE id = '$student_id' AND abbreviation = '$abbreviation'";
    $result_getClassroomID = mysqli_query($con, $query_getClassroomID);
    if (mysqli_num_rows($result_getClassroomID) == 1) {
        $row_getClassroomID = mysqli_fetch_assoc($result_getClassroomID);
        $student_classroom_id = $row_getClassroomID['student_classroom_id'];
    }
    ...
    $query2 = "SELECT teacher_notifications FROM student_classroom WHERE student_classroom_id = '$student_classroom_id'";

    $result2 = mysqli_query($con, $query2);

    while ($row = mysqli_fetch_assoc($result2)) {
        $teacher_notifications = $row['teacher_notifications'];

        if ($teacher_notifications == 1) {
            $query_update = "UPDATE student_classroom SET teacher_notifications = 0 WHERE student_classroom_id = '$student_classroom_id'";
...
<body class="w3-theme-l5">
    <div id="container" class="w3-card-4 w3-theme-l4">
        <h1 style="font-weight: bold;">Consult Student</h1>
        <div id="chat-box"></div>
        <div>
