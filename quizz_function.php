<?php 
    ...
    $query = "SELECT time_limit_min FROM quizz_data WHERE quizz_id = '$quizz_id'";
    $result = mysqli_query($con, $query);
    if ($result) {
        $row = mysqli_fetch_assoc($result);
...
<script>
    ...
    let incorrectAnswers = new Array(quizData.length);
    let selectedOption_list = new Array(quizData.length);
    let longAnswerToSave = new Array(quizData.length);
    ...
    function submitProcess() {
        ...
        for (let i = 0; i < quizData.length; i++) {
            if (incorrectAnswers[i] !== undefined) {
                ...
                answerToSave[i] = quizData[i].answer;
                score[i] = quizData[i].mark;
            }
            question_id[i] = quizData[i].question_id;
        }
        ...
        xhr.open("POST", "saveAnswerQuizz.php", true);
    ...
    function calResult() {
        ...
        for (let i = 0; i < quizData.length; i++) {
            if (!Array.isArray(quizData[i].options)) {
        ...
        if (!hasLongQuestion) {
            for (let i = 0; i < quizData.length; i++) {
                if (quizData[i].answer == answerToSave[i]) {
                    totalResult = totalResult + parseInt(quizData[i].mark);
                }
            }
            ...
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "saveResultQuizz.php", true);
            xhr.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
            xhr.send(JSON.stringify(dataToSend));
        }
    }
    ...
    function retryQuiz() {
        currentQuestion = 0;
        answerToSave = [];
        incorrectAnswers = new Array(quizData.length);
        selectedOption_list = new Array(quizData.length);
        longAnswerToSave = new Array(quizData.length);
        score = [];
        ...
        document.querySelectorAll('.side_bar_item').forEach((item, index) => {
            item.classList.remove('w3-text-teal');
            if (index === 0) {
                item.classList.add('w3-black'); 
                item.classList.add('w3-hover-black'); 
