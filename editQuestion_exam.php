<?php
    ...
    $answer_edit = urldecode($_GET['answer_edit']);
    $question_id = urldecode($_GET['question_id']);
    $_SESSION['question_id_current'] = $question_id;
    $option1 = isset($_GET['option1']) ? urldecode($_GET['option1']) : '';
    $option2 = isset($_GET['option2']) ? urldecode($_GET['option2']) : '';
    $option3 = isset($_GET['option3']) ? urldecode($_GET['option3']) : '';
    $option4 = isset($_GET['option4']) ? urldecode($_GET['option4']) : '';
...
<html>
    ...
    <body>
        <div class="w3-container w3-padding-64">
            <h2 class="w3-center"><b>Edit Question</b></h2>
            <div class="w3-card w3-padding w3-round-large" style="max-width: 600px; margin: 0 auto;">
                <form class="w3-container" action="saveQuestion_updateEdit_exam.php" method="POST">
                    <input type="hidden" name="noFirstSubmit" value="yes">
                    ...
                    <button onclick="confirmExit_edit()" class="w3-button w3-black w3-round-large w3-margin-bottom" type="button">Close</button>
                </form>
            </div>
        </div>
        ...
        <script>
            window.onload = function() {
                ...
                document.querySelector('textarea[name="question"]').value = "<?php echo $question; ?>";
                document.querySelector('input[name="mark"]').value = "<?php echo $mark; ?>";
                document.querySelector('select[name="correctAnswer"]').value = "<?php echo $answer_edit; ?>";
                ...
                var answer_edit = "<?php echo $answer_edit; ?>";
                if (document.getElementById('option1').value !== '') {
                    ...
                    updateOptionValue('option1');
                    updateOptionValue('option2');
                    updateOptionValue('option3');
                    updateOptionValue('option4');
                    correctAnswerSelect.value = answer_edit;
            ...
            function confirmExit_edit() {
                var confirmExit_edit = confirm("Are you sure you want to exit?");

                if (confirmExit_edit) {
                    window.location.href = 'editExam.php?exam_id=<?php echo $_SESSION['quizz_id_new']; ?>';
                }
            }
