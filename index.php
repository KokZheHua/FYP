<?php
    ...
    include("display_classroom.php");

    $id = $_SESSION['id'];
    $student_classroom = getClassroom();
    $student_classroom_hide = getClassroom_hide();
    ...
    <body class="w3-theme-l5">
        <div class="w3-top">
            <div class="w3-bar w3-white w3-wide w3-padding w3-card w3-theme-l4">
                ...
                <div class="w3-right">
                    <a href="consult_admin.php" id="focus_admin" class="w3-bar-item w3-button <?php echo ($unread_admin == 1 ? ' flag_css' : '') ?>">Consult Admin</a>
                    <a href="#" class="w3-bar-item w3-button" onclick="chooseColorTheme()">Color Theme</a>
                    <?php
                        if (isset($_SESSION['id'])) {
                            echo '<a href="profile.php?&previous_page=index" class="w3-bar-item w3-button">Profile</a>';
                ...
                <div class="w3-card-4 w3-margin w3-white w3-theme-l4">
                    <div class="w3-container">
                        ...
                        <div class="w3-row-padding">
                            <?php
                                if (!empty($student_classroom)) {
                                    foreach ($student_classroom as $classroom) {
                                        $query = "SELECT classroom_name, classroom_img FROM classroom_type WHERE abbreviation = '$classroom'";
                                        $result = mysqli_query($con, $query);
                                        if (mysqli_num_rows($result) > 0) {
                                            $totalItems = count($student_classroom);
                                            ...
                                            echo '<img src="' . $classroom_img . '" alt="' . $classroom_name . ' classroom" style="width:100%" class="w3-hover-opacity">';
                                                echo'<p><a href="openClassroomProcess.php?&classroom='. $classroom .'" class="w3-theme-l3 w3-button w3-padding-large w3-white w3-border classroom_btn"><b>'. $classroom_name .' »</b></a></p>';
                                                echo '<button class="w3-bar-item w3-white w3-button w3-hover-black" onclick="speak(\'' . $classroom_name . '\')"><b>&#128264; Hear</b></button>';
                                            echo '</div>';
                                        }
                                    }
                ...
                <div class="w3-card-4 w3-margin w3-white">
                    <div class="w3-container w3-theme-l4">
                                    ...
                                    foreach ($student_classroom as $classroom) {
                                        ...
                                        if ($count > 0) {
                                            ...
                                            echo'<div class="w3-container w3-margin" style="display: flex;">
                                                <a href="openClassroomProcess.php?&classroom='. $classroom .'" class="w3-theme-l3 w3-button w3-padding-large w3-white w3-border classroom_btn search_classroom"><b>'. $classroom_name .' »</b></a>
                        ...
                        <div class="w3-row-padding classroom_hide" id="classroom_hide" style="display:none;">
                            <?php
                                if (!empty($student_classroom_hide)) {
                                    foreach ($student_classroom_hide as $classroom) {
                                        $query = "SELECT classroom_name, classroom_img FROM classroom_type WHERE abbreviation = '$classroom'";
                                        $result = mysqli_query($con, $query);
                                        $count = mysqli_num_rows($result);
                                        if ($count > 0) {
                                            $totalItems = count($student_classroom_hide);
                                            ...
                                            echo '<img src="' . $classroom_img . '" alt="' . $classroom_name . ' classroom" style="width:100%" class="w3-hover-opacity">';
                                            echo'<div class="w3-container w3-margin" style="display: flex;">
                                                <a href="openClassroomProcess.php?&classroom='. $classroom .'" class="w3-button w3-padding-large w3-white w3-border classroom_btn search_classroom_hide"><b>'. $classroom_name .' »</b></a>
            ...
            <div class="w3-col l4 " style="padding-top: 60px">
                        ...
                        <?php 
                            if (isset($_SESSION['id'])) {
                                ...
                                echo '<table style="font-size: 20px;">';
                                    ...
                                    echo '<tr><td><b>Student ID</b></td><td style="padding-right:10px;">:</td><td><b>' . $id . '</b></td></tr>';
                                echo '</table>';
                            }
                            else{
                ...
                <div class="w3-card w3-margin">
                    ...
                    <ul class="w3-ul w3-white">
                        <?php
                            if (!empty($student_classroom)) {
                                $query = "SELECT abbreviation, student_classroom_id, unread_forum FROM student_classroom WHERE id = '$id' ORDER BY access_time_forum DESC LIMIT 5";
                                $result = mysqli_query($con, $query);
                                if (mysqli_num_rows($result) > 0) {
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        ...
                                        $student_classroom_id = $row['student_classroom_id']; 
                                        echo '<p><a href="social_zone.php?&abbreviation='. $abbreviation .'&student_classroom_id=' . $student_classroom_id . '" class=" w3-theme-l3 w3-button w3-padding-large w3-white w3-border' . ($unread_forum == 1 ? ' flag_css' : '') . '" style="width:70%"><b>Social Zone »</b></a>';
                                        echo '<button class="w3-bar-item w3-white w3-button w3-hover-black" style="width:30%" onclick="speak_social(\'' . $classroom_name . '\')"><b>&#128264; Hear</b></button></p>';
                                        echo '</li>';
                                    }
                                }                                    
                            }else{
        ...
        <script>
            ...
            function speak(speak_word) {
                const speech = new SpeechSynthesisUtterance(speak_word);
                speech.lang = 'en-US';
                speech.text = speak_word;
                window.speechSynthesis.speak(speech);
            }

            function speak_social(speak_word){
                const speech = new SpeechSynthesisUtterance();
                speech.lang = 'en-US';
                speech.text = speak_word + 'social zone';
                window.speechSynthesis.speak(speech);
            }
            ...
            function toggleSidebar(event, classroom, id, action) {
                ...
                xhr.open("POST", "showAndHide.php", true);
            ...
            document.addEventListener('DOMContentLoaded', function() {
                document.getElementById('focus_admin').focus();
            });
        </script>
