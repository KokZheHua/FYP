<?php 
    ...
    $query = "SELECT DISTINCT question_id FROM exam_question WHERE exam_id = '$exam_id'";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        foreach ($question_id_list as $question_id){
            ...
            $query = "SELECT user_answer_exam, score_mark FROM exam_question WHERE question_id = '$question_id' AND exam_id = '$exam_id'";
            $result = mysqli_query($con, $query);
            if(mysqli_num_rows($result) > 0){
                $row = mysqli_fetch_assoc($result);
...
<script>
    ...
    function addNewQuestion() {
        <?php
            ...
            $_SESSION['examTitle'] = 0;
            $_SESSION['examDescription'] = 0;
            $_SESSION['timeLimit'] = 0;
            $_SESSION['startTime'] = 0;
            $_SESSION['endTime'] = 0;
        ?>
        window.location.href='createQuestion_edit_exam.php';
    }
    ...
    function removeConfirmation(){
        ...
        if (confirm("Are you sure you want to remove the question?")) {
            window.location.href = 'removeQuestion_exam.php?question_id=' + question_id;
        }
    }
    ...
    function editQuestion() {
        ...
        if (answer_edit !== null){
            ...
            window.location.href = 'editQuestion_exam.php' + queryString;
        }else{
